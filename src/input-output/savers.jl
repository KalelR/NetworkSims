function saver(res, pvals, ptypes, savemode; equals="_", connector="-", sort=true, ignores=(), digits=20, allowedtypes = (Real, String, Symbol, Vector), kwargs...)
    if savemode == "allseparated"
        for (res_type, res_val) in res
            resfilename = name_result(res_type, pvals, ptypes; equals, connector, sort, ignores, digits, allowedtypes, kwargs...)
...
function savenames(resnames, pvals, ptypes, savemode; equals="_", connector="-", sort=true, ignores=(), digits=20, kwargs...)
    ...
    if savemode == "allseparated"
        for res_type in resnames
            resfilename = name_result(res_type, pvals, ptypes; equals, connector, sort, ignores, digits, allowedtypes)
