...
mutable struct SpikeTimeDetection2
...
mutable struct ParametersFHN2
    ...
    spiketimedetection :: SpikeTimeDetection2
    C :: Matrix{Float64}
    Csum :: Vector{Float64}
...
# mutable struct ParametersFHNAlpha
#     a :: Vector{Float64}
#     b :: Vector{Float64}
#     c :: Vector{Float64}
#     I :: Vector{Float64}
...
# function ParametersFHN(pvals, adjl, connsl, types_to_params) #TODO: solve this args..., lead to error before
    ...
    spiketimedetection = SpikeTimeDetection2(spiketimes, current_spikes_idxs, Vth, numstoredspiketimes)

    return ParametersFHN2(
    a, b, c, I, #units
    A, adjl, E, τs_vals, receivertypes, conductance, #coupling
    spiketimedetection, C, Csum, Icoup, ΔV, AΔV #prealloc
    )
end

@inbounds function fitzhugh_nagumo_mat_rule!(du, u, p, t)
    @unpack a, b, c, I, A, E, ΔV, Icoup, AΔV, C = p
    dVs = view(du, 1, :)
    dws = view(du, 2, :)
    Vs = view(u, 1, :)
    ws = view(u, 2, :)
    ...
    syn_sum!(p, t) #C = sum of conductances, 1xN
    diagmul!(Icoup, A, C) #Icoup, (allocates)
...
@inbounds function syn_sum!(p, t)
    ...
    spiketimes_cols = eachcol(spiketimes)
    # for i = 1:size(spiketimes, 1) #for each unit, calculate Csum (sum of currents) for each type of connection;
    for (i, spiketimes_i) in enumerate(spiketimes_cols)
        fill!(Csum, 0.0)
        receivers = p.adjl[i]::Vector{Int64}
        if isempty(receivers) == 0 view(C, i, :) .= 0.0; continue end #could change this to look at receiver types but oh well i have adjl already...
